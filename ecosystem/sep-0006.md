...
`email` | string | (optional) email which helps identify the transfer and manage if there is an error.
`partner_code` | string | (optional) code for partner using system. If you are a partner and are doing kyc yourself you will need to include this reference otherwise we will send a kyc email to unregistered depositers. Unless there is a partner code the name on the sending account should match the sending account name.
`method` | string | (optional) method of deposit e.g. sepa, swift, card, cash, sofort, ach etc
`amount` | string | (optional) helps identify the transfer and manage if there is an error. If for example the user sends a swift wire transfer with no email
